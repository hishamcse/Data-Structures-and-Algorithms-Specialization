...
// another way for this solution (from discussion forum):
// This is the hint for solving this problem.

// 1. Create a Directed Acyclic Graph(DAG) with directed edges when stock(i) < stock(j) (all the k points of stock(i)
// is less than all the k points of stock(j)).

// 2. Now convert the DAG into a bipartite graph by having 2 nodes per stock , eg) so stock(i) can be written as
// stock(i1) and stock(i2) and stock(j) as stock(j1) and stock(j2). If there was edge from stock(i)-> stock(j) in DAG ,
// then there is an edge in the bipartite graph from stock(i1)->stock(j2).

// 3. Now create this into a max flow by adding source and sink and capacities of 1 unit.
// 4. After computing max flow, find the no of vertices in the min cut ( no of vertices reachable from source) .
