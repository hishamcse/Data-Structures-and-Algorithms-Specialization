...
// almost same implementation of Diet.java but slight difference which makes the
// grader difference big(!!!!) . find out what(?????????)
...
public class AdAllocation {
    ...
    private int phase1() {
        ...
        if (a[m + 1][n + m + m] >= EPSILON * 1.0e3) return -1;
        return 0;
    }
    ...
    private int minRatioRule(int q) {
        ...
        for (int i = 0; i < m; i++) {
            if (a[i][q] <= EPSILON) {
                ...
                p = i;
            } else if (((a[i][n + m + m] / a[i][q]) + EPSILON <= (a[p][n + m + m] / a[p][q]))) {
                p = i;
            }
        }
        return p;
