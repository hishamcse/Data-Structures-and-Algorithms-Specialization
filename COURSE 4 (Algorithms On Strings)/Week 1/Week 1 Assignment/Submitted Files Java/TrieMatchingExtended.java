		...
		for (String pattern : patterns) {
			...
			for (int i = 0; i < pattern.length(); i++) {
				...
				if (i == pattern.length() - 1) {
					// it should be checked at last(not at first) of each iteration because at each iteration a string is
					// created upto ith char in the current node. If we check first it will not work because at that time
