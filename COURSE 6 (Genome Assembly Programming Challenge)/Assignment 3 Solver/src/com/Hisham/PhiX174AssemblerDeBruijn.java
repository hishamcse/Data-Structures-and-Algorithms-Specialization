...
public class PhiX174AssemblerDeBruijn {
    ...
    private static void bubbles_removal(Map<String, Set<String>> forward, Map<String, Set<String>> reverse) {
        for (String src : paths.keySet()) {
            Map<String, List<BubblePath>> all_w_vs_paths = paths.get(src);

            for (String des : all_w_vs_paths.keySet()) {
                List<BubblePath> w_v_paths = all_w_vs_paths.get(des);

                if (w_v_paths != null) {
                    List<Boolean> removed_paths = new ArrayList<>();
                    for (int l = 0; l < w_v_paths.size(); l++) {
                        removed_paths.add(false);
                    }

                    for (int k = 0; k < w_v_paths.size(); k++) {
                        int i = k;
                        while (i < removed_paths.size() && removed_paths.get(i)) {
                            i++;
                        }

                        for (int w = i + 1; w < w_v_paths.size(); w++) {
                            int j = w;
                            while (j < removed_paths.size() && removed_paths.get(j)) {
                                j++;
                            }

                            if (is_Disjoint(w_v_paths.get(i).path, w_v_paths.get(j).path, src, des)) {
                                int p = avg_Coverage_Minimum(w_v_paths.get(i), w_v_paths.get(j), i, j);

                                remove_Path(forward, reverse, w_v_paths.get(p).path);
    ...
    private static String nonIsolatedVertex(Map<String, Set<String>> adjOut) {
        for (String v : adjOut.keySet()) {
            if (adjOut.get(v) != null && adjOut.get(v).size() > 0) {
                return v;
            }
        }
        return null;
...
//        System.out.println(forward_graph);
//        System.out.println(reverse_graph);
